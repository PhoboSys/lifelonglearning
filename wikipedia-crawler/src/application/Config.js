const { omit, defaults } = require("lodash")
...
function _createConfig(logger) {
    ...
    const termsCollectorFile = `${projectRoot}/resources/${isRequireString(process.env.TERMS_COLLECTOR)}`;
    ...
    const isCollect = process.env.COLLECT;
    const neo4j = {
        port: process.env.NEO4J_PORT,
        uri: process.env.NEO4J_URI,
        login: process.env.NEO4J_LOGIN,
    ...
    return {
        isCollect: isCollect,
        termsCollectorFile: termsCollectorFile,
        neo4j: neo4j,
        lang: lang,
...
module.exports = {
    getConfig(logger) {
        if (!_config) {
            ...
            logger(omit(_config, ["security", "neo4j"]))
        }
        return _config
    }
};
