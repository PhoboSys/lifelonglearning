...
function initSigma(config) {
	...
	if (config.sigma && config.sigma.mouseProperties) {
		...
		mouseProps={
        minRatio: 0.25, // How far can we zoom out?
    ...
    $("#zoom").find("div.z").each(function () {
        ...
        a.click(function () {
			if (b == "center") {
		        ...
		        var a = sigInst._core;
	            sigInst.zoomTo(a.domElements.nodes.width / 2, a.domElements.nodes.height / 2, a.mousecaptor.ratio * ("in" == b ? 2.5 : 0.5));
			}
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        $GP.info_link.find("ul").html(f.join(""));
        $GP.info.animate({width:'show'},250);
        $GP.search.clean();
		$GP.cluster.hide();
        return !0
    }
